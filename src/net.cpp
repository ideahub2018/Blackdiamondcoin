...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        if (nHost == 1)
        {
            addrConnect = CService("213.1.214.237", 80); // showmyip.co.uk

            if (nLookup == 1)
            {
                CService addrIP("showmyip.co.uk", 80, false);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }
        ...
        {
            addrConnect = CService("74.208.44.161", 80); // www.showmemyip.com

            if (nLookup == 1)
            {
                CService addrIP("www.showmemyip.com", 80, false);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }
